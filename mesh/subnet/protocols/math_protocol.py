...
from mesh.proto import dht_pb2, math_pb2, runtime_pb2
...
class MathProtocol(mp.context.ForkProcess, ServicerBase):
    ...
    def run_in_background(self, await_ready: bool = True, timeout: Optional[float] = None) -> None:
        """
        Starts MathProtocol in a background process. If :await_ready:, this method will wait until
    ...
    def shutdown(self):
        if self.is_alive():
            ...
            if self.is_alive():
                logger.warning(
                    "MathProtocol did not shut down within the grace period; terminating it the hard way"
                )
                self.terminate()
        else:
            logger.warning("MathProtocol shutdown had no effect, the process is already dead")
    ...
    async def call_math(
        ...
        try:
            async with self.rpc_semaphore:
                ...
                response = await self.get_stub(p2p, peer).rpc_math(input)
                return response
        except Exception as e:
            logger.error(f"MathProtocol failed to stream from {peer}: {e}", exc_info=True)
            return

    def should_process_rpc(self, data: Any) -> bool:
        """
        Conditions to call rpc method
        """
        return True

    async def rpc_math(
        self, request: math_pb2.MathRequest, context: P2PContext
    ) -> math_pb2.MathResponse:
        """
        A peer wants us to perform an a math equation
        """
        """
        ...
        return math_pb2.MathResponse(
            peer=self.node_info,
            dht_time=get_dht_time(),
            output=float(eval(request.input)),
        )
